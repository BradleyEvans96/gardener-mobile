...
} from "../constants/DynamoDBConstants";
import { getEnvRaw } from "../helpers/env/getEnv";
...
export class DynamoDBService {
  private static readonly client: DynamoDBClient = new DynamoDBClient({
    region: getEnvRaw("AWS_DEFAULT_REGION"),
  });
  private static readonly dynamodb: DynamoDBDocumentClient =
    DynamoDBDocumentClient.from(this.client, {
      marshallOptions: {
  ...
  private static readonly callWithPagination = async (
    ...
    let lastEvaluatedKey: Record<string, NativeAttributeValue> | undefined =
      undefined;
    do {
      const data: ScanCommandOutput | QueryCommandOutput =
        await DynamoDBService.dynamodb.send(command(lastEvaluatedKey));
  ...
  private static async batchWrite(
    ...
    try {
      ...
      while (
        ...
        logger.info({
          message: `Unprocessed items found, retrying batch write. Attempt number: [${attempt}]`,
        });
      ...
      const message =
        "Error trying to batch write records using DynamoDB service";
      logger.error({ message, body: error });
      throw new Error(`${message}: [${error}]`);
    }
  }
  ...
  static async transactWrite(params: TransactWriteCommandInput): Promise<void> {
    try {
      ...
      const message =
        "Error trying to transact write records using DynamoDB service";
      logger.error({ message, body: error });
      throw new Error(`${message}: [${error}]`);
    }
  }
  ...
  private static async executeBatchGet(
    ...
    try {
      ...
      const message =
        "Error trying to batch get records using DynamoDB service";
      logger.error({ message, body: error });
      throw new Error(`${message}: [${error}]`);
    }
  }
  ...
  static async batchInsert<T>(
    ...
    const insertedItems: Record<string, T>[] = [];
    try {
      ...
      logger.error({
        message:
          "Error trying to batch insert records using DynamoDB service, removing partially inserted data",
        body: err,
      });
      ...
      throw new Error(
        `Error trying to batch write records using DynamoDB service: [${err}]`
      );
    }
  }
