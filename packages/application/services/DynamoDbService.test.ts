...
//       await expect(
//         DynamoDBService.scan({} as ScanCommandInput)
//       ).rejects.toThrow(
...
//       await expect(
//         DynamoDBService.transactWrite({} as TransactWriteCommandInput)
//       ).rejects.toThrow(
...
//       await expect(
//         DynamoDBService.batchGet(
//           [{ partitionKey: "mockPartitionKey", sortKey: "mockSortKey" }],
//           "mock-table-name"
//         )
//       ).rejects.toThrow(
//         `Error trying to batch get records using DynamoDB service: [${error}]`
//       );
//     });
//   });
...
//     it("should call batch delete if the batch insert fails", async () => {
//       mockDynamoDB.on(BatchWriteCommand).rejects();
//       await expect(
//         DynamoDBService.batchInsert(mockOffer, "mock-table-name")
//       ).rejects.toThrow();
//       expect(mockDynamoDB).toHaveReceivedCommandWith(BatchWriteCommand, {
//         RequestItems: {
//           "mock-table-name": mockOffer.map((item) => ({
...
//       await expect(
//         DynamoDBService.delete({} as DeleteCommandInput)
//       ).rejects.toThrow(
...
//       await expect(
//         DynamoDBService.query({} as QueryCommandInput)
//       ).rejects.toThrow(
